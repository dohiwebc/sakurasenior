...
(function () {
  ...
  /** Firebase Auth SDK があるページで auth 状態監視を二重に付けない */
  let firebaseAuthStateListenerAttached = false;
  ...
  function getFirebaseCompatAuth(fb) {
    try {
  ...
  function shouldDeferFirebaseReadForAuth(fb) {
    const auth = getFirebaseCompatAuth(fb);
    return Boolean(auth && !auth.currentUser);
  }

  function wireFirebaseAuthRefreshHooks() {
    if (firebaseAuthStateListenerAttached) return;
    const fb = typeof firebase !== "undefined" ? firebase : null;
    if (!fb?.apps?.length) return;
    const auth = getFirebaseCompatAuth(fb);
    if (!auth || typeof auth.onAuthStateChanged !== "function") return;
    firebaseAuthStateListenerAttached = true;
  ...
  function mountNav() {
      ...
      if (tab === "matches") {
        ...
        list.innerHTML = rows
          ...
          <article class="site-nav__notice ${n.unread ? "site-nav__notice--unread" : ""}" tabindex="0" role="button" data-notice-match="1"
            ...
            <div class="site-nav__notice-body">${escHtml(n.body)}</div>
          </article>`,
          )
          .join("");
    ...
    async function openNoticesPanel(initialTab = "matches") {
      ...
      if (shouldDeferFirebaseReadForAuth(fb)) {
        lastNoticeRes = null;
        summary.textContent = "ログイン後に通知を表示します。";
  ...
  function refreshSiteSettingsTutorialManualFromFirebase() {
    ...
    if (shouldDeferFirebaseReadForAuth(fb)) return;
    try {
      fb
        .database()
        .ref("siteSettings")
