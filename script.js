...
const OPPONENT_CATEGORIES = ["シニア", "U-18", "U-15", "ガールズ"];
...
const showGuestInMatchButtonEl = document.getElementById("showGuestInMatchButton");
...
const tournamentDefaultCategoryWrapEl = document.getElementById("tournamentDefaultCategoryWrap");
const tournamentDefaultOpponentCategoryEl = document.getElementById("tournamentDefaultOpponentCategory");
const tournamentParticipantsListEl = document.getElementById("tournamentParticipantsList");
...
const editTournamentDefaultCategoryWrapEl = document.getElementById("editTournamentDefaultCategoryWrap");
const editTournamentDefaultOpponentCategoryEl = document.getElementById("editTournamentDefaultOpponentCategory");
const editTournamentParticipantsListEl = document.getElementById("editTournamentParticipantsList");
...
const viewerShowGuestCheckboxEl = document.getElementById("viewerShowGuestCheckbox");
...
const matchOpponentCategoryWrapEl = document.getElementById("matchOpponentCategoryWrap");
const matchOpponentCategoryEl = document.getElementById("matchOpponentCategory");
const matchOpponentCategoryAutoHintEl = document.getElementById("matchOpponentCategoryAutoHint");
const editMatchOpponentCategoryWrapEl = document.getElementById("editMatchOpponentCategoryWrap");
const editMatchOpponentCategoryEl = document.getElementById("editMatchOpponentCategory");
const editMatchOpponentCategoryAutoHintEl = document.getElementById("editMatchOpponentCategoryAutoHint");
...
const lineupQuickRegisterButtonEl = document.getElementById("lineupQuickRegisterButton");
const lineupQuickRegisterAreaEl = document.getElementById("lineupQuickRegisterArea");
const lineupQuickPlayerNameEl = document.getElementById("lineupQuickPlayerName");
const lineupQuickRegisterErrorEl = document.getElementById("lineupQuickRegisterError");
const lineupQuickRegisterCancelButtonEl = document.getElementById("lineupQuickRegisterCancelButton");
const lineupQuickRegisterSubmitButtonEl = document.getElementById("lineupQuickRegisterSubmitButton");
...
function normalizeTournament(id, value = {}) {
  const participantPlayerIds = Array.isArray(value.participantPlayerIds)
    ? [...new Set(value.participantPlayerIds.filter((x) => typeof x === "string" && x))]
    : [];
  return {
    id,
    ...
    opponentCategoryMode: value.opponentCategoryMode === "fixed" ? "fixed" : "perMatch",
    defaultOpponentCategory: sanitizeOpponentCategory(value.defaultOpponentCategory),
...
function normalizeMatch(id, value = {}) {
  ...
  return {
    ...
    opponentCategory: value.opponentCategory ? sanitizeOpponentCategory(value.opponentCategory) : "",
...
function isGuestPlayerId(playerId) {
  return GUEST_PLAYER_IDS.has(String(playerId ?? ""));
}

function sanitizeOpponentCategory(value) {
  return OPPONENT_CATEGORIES.includes(value) ? value : OPPONENT_CATEGORIES[0];
}

function getTournamentOpponentCategoryMode(tournament) {
  return tournament?.opponentCategoryMode === "fixed" ? "fixed" : "perMatch";
}

function getTournamentDefaultOpponentCategory(tournament) {
  return sanitizeOpponentCategory(tournament?.defaultOpponentCategory);
}

function getTournamentParticipantIds(tournament) {
  if (!Array.isArray(tournament?.participantPlayerIds)) {
    return [];
  }
  return [...new Set(tournament.participantPlayerIds.filter((x) => typeof x === "string" && x))];
}
...
function getTournamentEligiblePlayers(tournament = getSelectedTournament()) {
  ...
  return activePlayers.filter((player) => isGuestPlayerId(player.id) || participantSet.has(player.id));
}

function readCheckedParticipantIdsFromList(listEl) {
  if (!listEl) return [];
  return Array.from(listEl.querySelectorAll('input[type="checkbox"][data-participant-player-id]:checked'))
    .map((input) => String(input.value || ""))
    .filter(Boolean);
}

function renderTournamentParticipantList(listEl, selectedIds) {
  if (!listEl) return;
  const selectablePlayers = getParticipantSelectablePlayers();
  const selected = new Set(selectedIds);
  listEl.innerHTML = "";
  if (!selectablePlayers.length) {
    listEl.innerHTML = '<p class="login-description no-margin">選択可能な選手がいません。</p>';
    return;
  }
  selectablePlayers.forEach((player) => {
    ...
    input.dataset.participantPlayerId = "1";
...
function resetTournamentCreateParticipantsSelection() {
  const defaultIds = getParticipantSelectablePlayers().map((player) => player.id);
  renderTournamentParticipantList(tournamentParticipantsListEl, defaultIds);
}

function resetTournamentEditParticipantsSelection(tournament) {
  const selectableIds = getParticipantSelectablePlayers().map((player) => player.id);
  const participantIds = getTournamentParticipantIds(tournament);
  const selectedIds = participantIds.length ? participantIds : selectableIds;
  renderTournamentParticipantList(editTournamentParticipantsListEl, selectedIds);
}

function getMatchEffectiveOpponentCategory(match, tournament = null) {
  if ((match?.opponentCategory ?? "") !== "") {
    return sanitizeOpponentCategory(match.opponentCategory);
  }
  const fallbackTournament = tournament ?? (match?.defaultOpponentCategory ? match : getSelectedTournament());
  return getTournamentDefaultOpponentCategory(fallbackTournament);
}

function syncTournamentCategoryModeVisibility(isEdit = false) {
  const modeEl = isEdit ? editTournamentOpponentCategoryModeEl : tournamentOpponentCategoryModeEl;
  const wrapEl = isEdit ? editTournamentDefaultCategoryWrapEl : tournamentDefaultCategoryWrapEl;
  if (!modeEl || !wrapEl) return;
  const isFixed = modeEl.value === "fixed";
  wrapEl.classList.toggle("hidden", !isFixed);
}

function syncMatchOpponentCategoryInputs() {
  const tournament = getSelectedTournament();
  const mode = getTournamentOpponentCategoryMode(tournament);
  const fixedCategory = getTournamentDefaultOpponentCategory(tournament);
  const isFixed = mode === "fixed";
  matchOpponentCategoryWrapEl?.classList.toggle("hidden", isFixed);
  editMatchOpponentCategoryWrapEl?.classList.toggle("hidden", isFixed);
  if (matchOpponentCategoryEl) {
    matchOpponentCategoryEl.disabled = isFixed || !isStaff();
    if (isFixed) matchOpponentCategoryEl.value = fixedCategory;
  }
  if (editMatchOpponentCategoryEl) {
    editMatchOpponentCategoryEl.disabled = isFixed || !isStaff();
    if (isFixed) editMatchOpponentCategoryEl.value = fixedCategory;
  }
  if (matchOpponentCategoryAutoHintEl) {
    matchOpponentCategoryAutoHintEl.textContent = isFixed
      ? `この大会は対戦カテゴリ固定（${fixedCategory}）です。`
      : "";
    matchOpponentCategoryAutoHintEl.classList.toggle("hidden", !isFixed);
  }
  if (editMatchOpponentCategoryAutoHintEl) {
    editMatchOpponentCategoryAutoHintEl.textContent = isFixed
      ? `この大会は対戦カテゴリ固定（${fixedCategory}）です。`
      : "";
    editMatchOpponentCategoryAutoHintEl.classList.toggle("hidden", !isFixed);
  }
}
...
function renderCheckMode() {
  ...
  const eligiblePlayers = getTournamentEligiblePlayers();
  const checkPlayers = eligiblePlayers.filter((player) => !isGuestPlayerId(player.id));
  if (!checkPlayers.length) {
  ...
  if (state.selectedTournamentId && state.selectedMatchId) {
    ...
    const unenteredPlayers = checkPlayers
      .filter((player) => {
  ...
  if (state.selectedTournamentId && currentMatches.length) {
    ...
    if (tournamentHasAnyRecord) {
      const missingPlayers = checkPlayers
        .filter((player) =>
...
function applyPermission() {
  ...
  if (tournamentOpponentCategoryModeEl) tournamentOpponentCategoryModeEl.disabled = !canEdit;
  if (tournamentDefaultOpponentCategoryEl) tournamentDefaultOpponentCategoryEl.disabled = !canEdit;
  tournamentParticipantsListEl
    ?.querySelectorAll('input[type="checkbox"][data-participant-player-id]')
    .forEach((input) => {
      input.disabled = !canEdit;
    });
  if (selectAllTournamentParticipantsButtonEl) selectAllTournamentParticipantsButtonEl.disabled = !canEdit;
  if (clearTournamentParticipantsButtonEl) clearTournamentParticipantsButtonEl.disabled = !canEdit;
  if (editTournamentOpponentCategoryModeEl) editTournamentOpponentCategoryModeEl.disabled = !canEdit;
  if (editTournamentDefaultOpponentCategoryEl) editTournamentDefaultOpponentCategoryEl.disabled = !canEdit;
  editTournamentParticipantsListEl
    ?.querySelectorAll('input[type="checkbox"][data-participant-player-id]')
    .forEach((input) => {
      input.disabled = !canEdit;
    });
  if (selectAllEditTournamentParticipantsButtonEl) selectAllEditTournamentParticipantsButtonEl.disabled = !canEdit;
  if (clearEditTournamentParticipantsButtonEl) clearEditTournamentParticipantsButtonEl.disabled = !canEdit;
  ...
  lineupQuickRegisterButtonEl?.classList.toggle("hidden", !canEdit);
  if (midMatchAddPlayerButtonEl) {
    midMatchAddPlayerButtonEl.disabled = !canEdit;
  }
  syncTournamentCategoryModeVisibility(false);
  syncTournamentCategoryModeVisibility(true);
  syncMatchOpponentCategoryInputs();
...
function login(role, password) {
  ...
  state.role = role;
  sessionStorage.setItem("sakura_role", role);
...
function logout() {
  state.role = null;
  sessionStorage.removeItem("sakura_role");
...
function getActiveOnFieldCount() {
  return getTournamentEligiblePlayers().filter((player) => !isBenched(player.id)).length;
}
...
function getVisiblePlayers() {
  const activePlayers = getTournamentEligiblePlayers().filter((player) =>
    state.showGuestInMatch ? true : !isGuestPlayerId(player.id),
  );
...
function getAllPlayersTotalPlays() {
  return getTournamentEligiblePlayers().reduce((sum, player) => {
...
function renderRecordSummary() {
  const activePlayers = getTournamentEligiblePlayers();
  ...
  const parts = [
    ...
    match?.stage,
    match ? getMatchEffectiveOpponentCategory(match, tournament) : "",
    match?.opponent,
  ].filter(Boolean);
...
function renderPlayerManagementList() {
  ...
  players.forEach((player, index) => {
    ...
    const isGuest = isGuestPlayerId(player.id);
    if (isGuest) row.classList.add("player-mgmt-card--guest");
    ...
    const guestBadge = isGuest ? `<span class="player-mgmt-chip player-mgmt-chip--guest">常設</span>` : "";
    head.innerHTML = `
    ...
    if (isStaff()) {
      ...
      if (isGuest) {
        primaryActions.append(saveButton);
      } else {
...
function getViewerFilteredMatches() {
  ...
  return currentMatches.filter((match) => {
    ...
    const category = getMatchEffectiveOpponentCategory(match, selectedTournament);
    const categoryOk = state.viewerOpponentCategoryFilter === "all" || category === state.viewerOpponentCategoryFilter;
...
function getMatchLabel(match) {
  ...
  const category = getMatchEffectiveOpponentCategory(match, tournament);
  return `${orderLabel}${tournamentLabel} / ${match.stage || "区分未設定"} / ${category} / ${match.opponent || "対戦相手未設定"}`;
}
...
function renderViewerPlayerSelect() {
  ...
  players
    ...
    .filter((player) => state.viewerShowGuest || !isGuestPlayerId(player.id))
    .forEach((player) => {
...
function renderViewerOpponentCategoryFilter() {
  viewerOpponentCategoryFilterEl.innerHTML = "";
  const allOption = document.createElement("option");
  allOption.value = "all";
  allOption.textContent = "すべてのカテゴリ";
  viewerOpponentCategoryFilterEl.appendChild(allOption);

  const stage = state.viewerStageFilter;
  const selectedTournament = getSelectedTournament();
  const matchesForCategories = currentMatches.filter((match) => {
    if (stage === "all") return true;
    return (match.stage ?? "") === stage;
  });
  const categories = Array.from(
    new Set(matchesForCategories.map((match) => getMatchEffectiveOpponentCategory(match, selectedTournament))),
  );
  categories.forEach((category) => {
    const option = document.createElement("option");
    option.value = category;
    option.textContent = category;
    viewerOpponentCategoryFilterEl.appendChild(option);
  });
  const available = ["all", ...categories];
  if (!available.includes(state.viewerOpponentCategoryFilter)) {
    state.viewerOpponentCategoryFilter = "all";
  }
...
function getViewerNameSortIndex(row) {
  const player = players.find((p) => p.id === row.id);
  if (player && Number.isFinite(player.order)) {
    return Number(player.order);
  }
  const plainName = getViewerPlainPlayerNameForSort(row);
...
function sortViewerRowsWithKeyOrder(rows, sortKey, sortOrder) {
  ...
  return [...rows].sort((a, b) => {
    if (key === "name") {
      ...
      const ia = getViewerNameSortIndex(a);
      const ib = getViewerNameSortIndex(b);
      if (ia !== ib) {
...
function syncViewerSortControls() {
  ...
  if (viewerShowGuestCheckboxEl) viewerShowGuestCheckboxEl.checked = state.viewerShowGuest;
...
async function buildTournamentViewerSnapshot(tournamentId) {
  ...
  const rows = players
    ...
    .filter((row) => state.viewerShowGuest || !isGuestPlayerId(row.id))
...
async function buildAllMatchesViewerSnapshot() {
  ...
  for (const tournament of currentTournaments) {
    ...
    matches.forEach((match) => {
      ...
      allMatches.push({
        ...
        opponentCategoryMode: getTournamentOpponentCategoryMode(tournament),
        defaultOpponentCategory: getTournamentDefaultOpponentCategory(tournament),
      });
  ...
  const filteredMatches = allMatches.filter((match) => {
    ...
    const stageOk = state.viewerStageFilter === "all" || (match.stage ?? "") === state.viewerStageFilter;
    const opponentOk = state.viewerOpponentFilter === "all" || (match.opponent ?? "") === state.viewerOpponentFilter;
    const category = getMatchEffectiveOpponentCategory(match, match);
    const categoryOk = state.viewerOpponentCategoryFilter === "all" || category === state.viewerOpponentCategoryFilter;
...
function renderViewerContent() {
  ...
  renderViewerOpponentCategoryFilter();
  const filteredMatches = getViewerFilteredMatches();
  ...
  if (!filteredMatches.length) {
    state.viewerLastRenderedRows = [];
    state.viewerLastSelectedMatchIds = [];
    setViewerEmptyState("現在の区分・対戦相手・対戦カテゴリ条件に一致する試合がありません。条件を変更してみてください。");
    viewerTotalsEl.innerHTML = "";
    viewerRankingsEl.innerHTML = "";
    renderViewerTable([]);
    viewerDetailSummaryEl.textContent = "";
  ...
  const selectedTournamentLabel =
    !isAllMatchesSelected && selectedTournament
      ? ` / 大会: ${selectedTournament.name}${selectedTournament.date ? `（${selectedTournament.date}）` : ""}`
      : "";
  viewerSummaryEl.textContent = `${selectedMatchIds.length}試合 / ${selectedPlayerText}${selectedTournamentLabel}`;
...
function getRosterStartersAndBench() {
  ...
  const starterIdsOrdered = Array.isArray(match?.startingMemberIds)
    ? match.startingMemberIds.filter((id) => Boolean(activeById[id]))
    : [];
  const starters = [];
  const benches = [];
...
function renderMatchRosterPanels() {
  ...
  if (metaEl) {
    ...
    if (match) {
      parts.push(`対戦カテゴリ: ${getMatchEffectiveOpponentCategory(match, tournament)}`);
    }
    if (tournament?.name) {
...
function openSubstitutionRecordModal(benchPlayerId) {
  ...
  const outs = getTournamentEligiblePlayers().filter((p) => !isBenched(p.id));
  if (!outs.length) {
    openMatchFlowModal({
      title: "途中交代を記録できません",
      bodyHtml:
...
function seedLineupDraftFromMatchAndRecords() {
  ...
  if (matchHasValidSavedLineup(match)) {
    state.lineupDraftIds = match.startingMemberIds.filter((id) => eligibleIds.has(id));
    return;
  }
  ...
  const fromRecords = eligiblePlayers
...
function renderLineupPickPanel() {
  ...
  const eligiblePlayers = getTournamentEligiblePlayers();
  const regularPlayers = eligiblePlayers.filter((p) => !isGuestPlayerId(p.id));
  const guestPlayers = eligiblePlayers.filter((p) => isGuestPlayerId(p.id));
  const lineupPlayers = guestPlayers.length ? [...regularPlayers, null, ...guestPlayers] : regularPlayers;
  lineupPlayerCheckboxListEl.innerHTML = "";

  const buildCheckRow = (player) => {
    const label = document.createElement("label");
    label.className = "lineup-check-row" + (isGuestPlayerId(player.id) ? " lineup-check-row--guest" : "");
  ...
  lineupPlayers.forEach((player) => {
    if (player === null) {
      ...
      sep.className = "lineup-guest-separator";
      lineupPlayerCheckboxListEl.appendChild(sep);
      return;
    }
    lineupPlayerCheckboxListEl.appendChild(buildCheckRow(player));
  });
...
function renderTournamentOptions(tournaments) {
  ...
  tournaments.forEach((item) => {
    ...
    option.textContent = item.date ? `${item.name}（${item.date}）` : item.name;
    tournamentSelectEl.appendChild(option);
  });
...
function renderMatchOptions(matches) {
  ...
  sorted.forEach((item) => {
    ...
    const category = getMatchEffectiveOpponentCategory(item, tournament);
    option.textContent = `${orderLabel}${tournamentLabel} / ${item.stage || "区分未設定"} / ${category} / ${item.opponent || "対戦相手未設定"}`;
    matchSelectEl.appendChild(option);
  });
...
function fillMatchMetaInputs(match) {
  ...
  if (!match) {
    ...
    matchOpponentCategoryEl.value = fixedCategory;
    syncMatchOpponentCategoryInputs();
    return;
  }
  ...
  matchOpponentCategoryEl.value = getMatchEffectiveOpponentCategory(match, tournament);
  syncMatchOpponentCategoryInputs();
}

function fillTournamentMetaInputs(tournament) {
  if (!tournament) {
    tournamentDateEl.value = "";
    tournamentMatchTypeEl.value = "大会";
    tournamentOpponentCategoryModeEl.value = "perMatch";
    tournamentDefaultOpponentCategoryEl.value = OPPONENT_CATEGORIES[0];
    resetTournamentCreateParticipantsSelection();
    syncTournamentCategoryModeVisibility(false);
    syncMatchOpponentCategoryInputs();
    return;
  }
  ...
  tournamentOpponentCategoryModeEl.value = getTournamentOpponentCategoryMode(tournament);
  tournamentDefaultOpponentCategoryEl.value = getTournamentDefaultOpponentCategory(tournament);
  resetTournamentCreateParticipantsSelection();
  syncTournamentCategoryModeVisibility(false);
  syncMatchOpponentCategoryInputs();
}
...
function fillTournamentEditInputs(tournament) {
  if (!tournament) {
    ...
    editTournamentOpponentCategoryModeEl.value = "perMatch";
    editTournamentDefaultOpponentCategoryEl.value = OPPONENT_CATEGORIES[0];
    resetTournamentEditParticipantsSelection(null);
    syncTournamentCategoryModeVisibility(true);
    return;
  }
  ...
  editTournamentOpponentCategoryModeEl.value = getTournamentOpponentCategoryMode(tournament);
  editTournamentDefaultOpponentCategoryEl.value = getTournamentDefaultOpponentCategory(tournament);
  resetTournamentEditParticipantsSelection(tournament);
  syncTournamentCategoryModeVisibility(true);
}
...
function fillMatchEditInputs(match) {
  ...
  if (!match) {
    ...
    editMatchOpponentCategoryEl.value = fixedCategory;
    syncMatchOpponentCategoryInputs();
    return;
  }
  editOpponentEl.value = match.opponent ?? "";
  editMatchStageEl.value = match.stage ?? "予選";
  editMatchOpponentCategoryEl.value = getMatchEffectiveOpponentCategory(match, tournament);
  syncMatchOpponentCategoryInputs();
}
...
async function ensureGuestPlayerExists() {
  if (!db) return;
  for (const [index, guest] of GUEST_PLAYER_SLOTS.entries()) {
    const snap = await getPlayerRef(guest.id).once("value");
...
async function loadPlayersFromRealtimeDatabase() {
  ...
  try {
    ...
    resetTournamentCreateParticipantsSelection();
    resetTournamentEditParticipantsSelection(getSelectedTournament());
    renderPlayerManagementList();
    renderTabs();
    await loadRecordsForMatch();
    renderViewerContent();
...
function getMatchFormMeta() {
  ...
  const opponentCategory =
    mode === "fixed"
      ? getTournamentDefaultOpponentCategory(tournament)
      : sanitizeOpponentCategory(matchOpponentCategoryEl.value);
  return {
...
function buildRecordsPayload() {
  const records = getTournamentEligiblePlayers().map((player) => {
...
function setCreatePlayerError(msg) {
  if (!createPlayerErrorEl) return;
  if (msg) {
    createPlayerErrorEl.textContent = msg;
    createPlayerErrorEl.classList.remove("hidden");
  } else {
    createPlayerErrorEl.textContent = "";
    createPlayerErrorEl.classList.add("hidden");
  }
}
...
async function createPlayer() {
  ...
  if (!name) {
    setCreatePlayerError("選手名を入力してください");
...
async function updatePlayerDetails(playerId, nextName, nextGrade, nextRole, nextHandedness, nextMemo) {
  ...
  // 学年は任意。助っ人更新時は常に空で保持する。
  const grade = isGuestPlayerId(playerId) ? "" : nextGrade;
...
async function applyPlayerDisplayOrder(playerId, newIndexRaw) {
  ...
  let newIndex = Math.floor(Number(newIndexRaw)) - 1;
...
async function createTournament() {
  ...
  const opponentCategoryMode = tournamentOpponentCategoryModeEl.value === "fixed" ? "fixed" : "perMatch";
  const defaultOpponentCategory = sanitizeOpponentCategory(tournamentDefaultOpponentCategoryEl.value);
  const participantPlayerIds = readCheckedParticipantIdsFromList(tournamentParticipantsListEl);
  if (!name) {
...
async function updateSelectedTournament() {
  ...
  const opponentCategoryMode = editTournamentOpponentCategoryModeEl.value === "fixed" ? "fixed" : "perMatch";
  const defaultOpponentCategory = sanitizeOpponentCategory(editTournamentDefaultOpponentCategoryEl.value);
  const participantPlayerIds = readCheckedParticipantIdsFromList(editTournamentParticipantsListEl);
...
async function updateSelectedMatch() {
  ...
  const tournament = getSelectedTournament();
  const isFixedCategoryTournament = getTournamentOpponentCategoryMode(tournament) === "fixed";
  const opponentCategory = isFixedCategoryTournament
    ? getTournamentDefaultOpponentCategory(tournament)
    : sanitizeOpponentCategory(editMatchOpponentCategoryEl.value);
...
async function saveCurrentMatchRecords(isAutoSave = false) {
  ...
  try {
    ...
    const updates = {};
    Object.entries(matchPatch).forEach(([key, value]) => {
      updates[`${DB_PATHS.matches}/${state.selectedTournamentId}/${state.selectedMatchId}/${key}`] = value;
    });
    updates[`${DB_PATHS.records}/${state.selectedTournamentId}/${state.selectedMatchId}`] = buildRecordsPayload();
...
selectAllTournamentParticipantsButtonEl?.addEventListener("click", () => {
  ...
  renderTournamentParticipantList(tournamentParticipantsListEl, ids);
  applyPermission();
});

clearTournamentParticipantsButtonEl?.addEventListener("click", () => {
  renderTournamentParticipantList(tournamentParticipantsListEl, []);
  applyPermission();
});

selectAllEditTournamentParticipantsButtonEl?.addEventListener("click", () => {
  const ids = getParticipantSelectablePlayers().map((player) => player.id);
  renderTournamentParticipantList(editTournamentParticipantsListEl, ids);
  applyPermission();
});

clearEditTournamentParticipantsButtonEl?.addEventListener("click", () => {
  renderTournamentParticipantList(editTournamentParticipantsListEl, []);
  applyPermission();
});
...
function openLineupQuickRegister() {
  lineupQuickRegisterAreaEl.classList.remove("hidden");
  lineupQuickRegisterButtonEl.classList.add("hidden");
  lineupQuickPlayerNameEl.value = "";
  lineupQuickRegisterErrorEl.classList.add("hidden");
  lineupQuickPlayerNameEl.focus();
}

function closeLineupQuickRegister() {
  lineupQuickRegisterAreaEl.classList.add("hidden");
  lineupQuickRegisterButtonEl.classList.remove("hidden");
  lineupQuickRegisterErrorEl.classList.add("hidden");
  lineupQuickPlayerNameEl.value = "";
}

async function submitLineupQuickRegister() {
  const name = lineupQuickPlayerNameEl.value.trim();
  if (!name) {
    lineupQuickRegisterErrorEl.textContent = "選手名を入力してください";
    lineupQuickRegisterErrorEl.classList.remove("hidden");
    lineupQuickPlayerNameEl.focus();
    return;
  }
  lineupQuickRegisterErrorEl.classList.add("hidden");
  ...
  closeLineupQuickRegister();
  renderLineupPickPanel();
  setSaveStatus(`${name} を登録してスタメンに追加しました`);
}

lineupQuickRegisterButtonEl?.addEventListener("click", () => {
  openLineupQuickRegister();
});

lineupQuickRegisterCancelButtonEl?.addEventListener("click", () => {
  closeLineupQuickRegister();
});

lineupQuickRegisterSubmitButtonEl?.addEventListener("click", () => {
  void submitLineupQuickRegister();
});

lineupQuickPlayerNameEl?.addEventListener("keydown", (e) => {
  if (e.key === "Enter") void submitLineupQuickRegister();
});
...
viewerShowGuestCheckboxEl?.addEventListener("change", () => {
  state.viewerShowGuest = viewerShowGuestCheckboxEl.checked;
  if (!state.viewerShowGuest && isGuestPlayerId(state.viewerSelectedPlayerId)) {
    state.viewerSelectedPlayerId = "all";
  }
  renderViewerContent();
});
...
showGuestInMatchButtonEl?.addEventListener("click", () => {
  state.showGuestInMatch = !state.showGuestInMatch;
  showGuestInMatchButtonEl.textContent = state.showGuestInMatch ? "助っ人を非表示" : "助っ人を表示";
  renderTabs();
  renderStats();
});
...
// リロード後もログイン状態を維持する（sessionStorage はタブを閉じると消える）
const _savedRole = sessionStorage.getItem("sakura_role");
if (_savedRole === "player" || _savedRole === "staff") {
  login(_savedRole, _savedRole === "staff" ? PASSWORDS.staff : "");
}
