...
function renderPlayerManagementList() {
  ...
  players.forEach((player, index) => {
    ...
    const isGuest = player.id === GUEST_PLAYER_ID;
...
function renderViewerPlayerSelect() {
  ...
  players
    ...
    .filter((player) => state.viewerShowGuest || player.id !== GUEST_PLAYER_ID)
    .forEach((player) => {
  ...
  const availableIds = ["all", ...players.filter((player) => player.active !== false).filter((player) => state.viewerShowGuest || player.id !== GUEST_PLAYER_ID).map((player) => player.id)];
...
async function buildTournamentViewerSnapshot(tournamentId) {
  ...
  const rows = players
    ...
    .filter((row) => state.viewerShowGuest || row.id !== GUEST_PLAYER_ID)
...
function renderLineupPickPanel() {
  ...
  const regularPlayers = players.filter((p) => p.active !== false && p.id !== GUEST_PLAYER_ID);
  const guestPlayer = players.find((p) => p.id === GUEST_PLAYER_ID);
  const lineupPlayers = guestPlayer ? [...regularPlayers, null, guestPlayer] : regularPlayers;
  lineupPlayerCheckboxListEl.innerHTML = "";

  const buildCheckRow = (player) => {
    const label = document.createElement("label");
    label.className = "lineup-check-row" + (player.id === GUEST_PLAYER_ID ? " lineup-check-row--guest" : "");
...
async function ensureGuestPlayerExists() {
  if (!db) return;
  const snap = await getPlayerRef(GUEST_PLAYER_ID).once("value");
  if (!snap.exists()) {
    await getPlayerRef(GUEST_PLAYER_ID).set({
      id: GUEST_PLAYER_ID,
      name: "助っ人",
      grade: "",
      role: "",
...
async function updatePlayerDetails(playerId, nextName, nextGrade, nextRole, nextHandedness, nextMemo) {
  ...
  const grade = playerId === GUEST_PLAYER_ID ? "" : nextGrade;
...
viewerShowGuestCheckboxEl?.addEventListener("change", () => {
  ...
  if (!state.viewerShowGuest && state.viewerSelectedPlayerId === GUEST_PLAYER_ID) {
    state.viewerSelectedPlayerId = "all";
  }
  renderViewerContent();
});
